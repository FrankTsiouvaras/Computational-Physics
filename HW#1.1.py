...
# df = [-f(x+2h) + 8f(x+h) - 8f(x-h) + f(x-2h)]/(22h)
...
exact_cos_01 = np.float32(-np.sin(np.float32(0.1)))
exact_cos_10 = np.float32(-np.sin(np.float32(10.0)))
exact_exp_01 = np.float32(np.exp(np.float32(0.1)))
exact_exp_10 = np.float32(np.exp(np.float32(10.0)))
...
plt.loglog(h_ref, eps/h_ref, "k--", label="roundoff, eps/h");
plt.legend();
